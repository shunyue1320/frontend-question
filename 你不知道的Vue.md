...
工厂模式：vue中的createElement传入ast标签描述生成vNode
观察者模式：watcher & dep 的关系
代理模式：代理模式new proxy, vuex, 通过 vue.xx 操作 vue.data.xx
装饰模式：vue2中的@装饰器用法，作用是对原有功能增强
中介者模式：通过一个统一的接口与不同部分通信 如：vuex
策略模式：策略模式指对像有某个行为，但是在不同的场景中，该行为有不同的实现方案
